...
- If verification failed: the full output of `Rscript scripts/015-verify-ingestion.R`.
...
2. Stage the source file at `data/raw/<slug>/` and write `scripts/010-ingest-<slug>.R`.
...
- `suppressPackageStartupMessages({ library(...) })` at the top of scripts.
- Function names descriptive, not abbreviated. No `p` or `k` for variables.
...
- Every ingestion script writes a `provenance.yml` with SHA256, retrieval date, and licensing.
...
- Every exported function needs roxygen docs with `@param`, `@return`, `@export`.
...
- Target `main`.
- Pass `R CMD check` locally (CI runs this on macOS, Windows, and Ubuntu).
