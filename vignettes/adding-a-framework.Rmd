...
Extending `cybedtools` with a framework beyond the current eleven (NICE, DCWF, SFIA, ECSF, Cyber.org K-12, CSTA, CSEC2017, DigComp 3.0, CyQUAL, CCSSF, OTCCF) is a six-step process. This vignette walks through the steps using a hypothetical "Framework X" to make the pattern concrete.
...
Both paths assert `cybed:OrganizingUnit` so cross-framework queries reach all eleven frameworks via the abstract type. Only `build_role_node()` additionally asserts `cybed:Role`, restricting workforce-only queries appropriately.
