...
# framework_summary ships as lazy-loaded package data, so this always
# evaluates without a staged graph. Keeps the role-framework list below
# from drifting out of sync with a re-ingested framework.
fs_ai <- cybedtools::framework_summary
role_fw_names_ai   <- fs_ai$framework_name[!is.na(fs_ai$role_count)]
role_fw_list_ai    <- paste(role_fw_names_ai, collapse = ", ")
nonrole_fw_count_ai <- sum(is.na(fs_ai$role_count))
```
...
`cybed:OrganizingUnit` reaches every framework in the corpus. `cybed:Role` reaches only the frameworks that declare roles: `r role_fw_list_ai`. A query written against `cybed:Role` when the question is corpus-wide silently drops the other `r nonrole_fw_count_ai` frameworks.
