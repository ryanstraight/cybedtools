...
# evaluates without a staged graph. Keeps the total-statement figure
# below from drifting out of sync with a re-ingested framework.
statement_total_fmt <- format(
  sum(cybedtools::framework_summary$element_count_strict),
  big.mark = ","
)
```
...
It is one-sided. Statement IRIs do not move. They are the codes people cite, there are `r statement_total_fmt` of them (parent statements, strict count), and a framework's own numbering is what a reader looks up.
