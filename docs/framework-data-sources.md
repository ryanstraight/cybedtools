...
A separate hazard runs the other way, inside a single framework rather than across two. Some frameworks number their organizing units and their statements out of one id space, so a unit code and a statement code can be the same string. DCWF does this: its work-role codes and its task/KSA numbers share a numeric range. Cyber.org K-12 does it structurally, by naming a grade-band x sub-concept cell after the standard it holds. A framework prefix does not separate those, because both nodes are in the same framework. Unit IRIs in those two frameworks therefore carry a discriminator declared as `unit_iri_prefix` in `docs/framework-invariants.yml`: DCWF work role 462 is `dcwf:role-462` while statement 462 stays `dcwf:462`, and a Cyber.org cell is `cyberorg:cell-K-2.SEC.AUTH` while the standard stays `cyberorg:K-2.SEC.AUTH`. The unit's printed code is carried as a `schema:identifier` literal. `scripts/026-verify-graph.R` fails the build if any framework ever mints a fused IRI again.
...
**Ingest.** `Rscript scripts/010-ingest-scywf.R`

**Notes.** NICE-shaped: 40 job roles in 12 specialty areas and 5 categories, 646 tasks, 430 knowledge statements and 343 skill statements, and 24 competency areas. Role cards print Task, Knowledge and Skill codes only, so the 4,794 role-to-statement links come from the cards and the statement text from Appendix B. Every card code resolves to an Appendix B statement, and one statement (S1505) is on no card. Job roles are `scywf:JobRole` and `cybed:Role`, keyed by NCA's Job Role ID. Statements are keyed by NCA's printed codes, and skills carry NCA's "Technical?" column as `scywf:technical`. Competency areas, specialty areas and categories are organizing units that are not roles. Category ICSOT and specialty area ICSOT share one ID, so the group units carry an IRI discriminator (`scywf:category-ICSOT`, `scywf:specialty-ICSOT`) and keep the printed ID as `schema:identifier`. The edges `scywf:inSpecialtyArea` (role to specialty area) and `scywf:inCategory` (specialty area to category) are cybedtools-derived from the card fields and the role IDs. They are not NCA content. Extraction rebuilds table cells from word coordinates, and the only text normalisation is whitespace and line-break hyphenation, both documented in the ingest script. Every carried string (3,290 of them) is then looked up in the independent xpdf extraction, and the ingest stops on any miss. The sub-point parser is off. The category and specialty area descriptions of the document's Tables 1 and 2 are not carried yet. The first public data release was cut before this framework joined, so `docs/data-release.yml` holds it for the next release.
