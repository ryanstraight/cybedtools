...
The "Roles" column reports each framework's top-level organizing unit, whatever the framework calls it: work roles for NICE / DCWF / ECSF, skills for SFIA, level-concept buckets for CSTA, grade-band cells for Cyber.org K-12, Knowledge Areas for CSEC2017, competence areas for DigComp 2.2. The schema collapses these distinct structural commitments under a single `cybed:Role` abstraction. See `docs/framework-data-sources.md` for the per-framework structural mapping.
...
### Per-organizing-unit density varies by `r density_ratio_x`x across the corpus

`r density_max_name` expresses `r format(density_max_value, nsmall = 1)` elements per its top-level unit; `r density_min_name` expresses `r format(density_min_value, nsmall = 1)`. The spread reflects framework design philosophy more than relative completeness: NICE / DCWF are granular by intent as the basis for hiring and training pipelines; ENISA's ECSF and JRC's DigComp are intentionally high-level as interoperability frames and citizen self-assessment instruments. Treat per-unit density as a comparison aid, not a quality claim.
...
`r nice_top5_text`. Curricula that "cover NICE" by surveying these five look thorough. Curricula that cover the long tail of `r fs$role_count[fs$framework_slug == "nice-v2"]` roles look thin by element count alone. This is a property of NICE's internal weighting, not a finding about the broader corpus.
