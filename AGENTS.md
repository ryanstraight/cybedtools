...
CSEC2017, JRC DigComp 3.0, the UK's CyBOK). The
...
outputs as competing with or replacing NICE, SFIA, or any other
upstream framework.
...
If `framework_metadata()` returns a 2-row tibble with US and EU
jurisdictions, the install is sound and you can move to real data.
If it errors with an `rdflib`/`librdf` message, the system library is
missing. Linux: `apt-get install librdf0-dev libraptor2-dev
librasqal3-dev`. macOS: `brew install librdf`. Windows: ships with the
rdflib binary, no extra step usually required.
...
All package errors use `rlang::abort()` with classed conditions, so you
can branch on class instead of regex-matching messages:
...
- `tests/testthat/helper-fixture-graph.R`: synthetic graph encoding
  the structural invariants the SPARQL helpers must satisfy. Read this
  when adding helpers, then mirror the pattern.
- `vignettes/articles/namespace-architecture.Rmd`: two-tier `cybed:` schema design (rendered as a pkgdown article).
...
1. Compose 1-N single-BGP calls via `sparql_pairs()` /
   `sparql_subjects()`.
2. Join with `dplyr::left_join` / `dplyr::inner_join` /
   `dplyr::semi_join`.
...
4. Add tests against `tests/testthat/helper-fixture-graph.R` covering
   happy path + edge cases (orphan elements, duplicate triples,
   empty graph).
...
- `here::here()` for paths in scripts. Avoid hardcoded paths.
...
- Errors via `rlang::abort()` with bullet body and classed condition.
  See existing `cybedtools_*` classes.
...
- It does not (currently) implement any posthumanist analytical
  overlay. That overlay was deliberately deferred to a separate
  downstream package (`cybedposthuman`) so this package stays focused on
  framework representation and querying.
