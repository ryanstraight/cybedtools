...
The `anti_join` is required, not optional. `cybed:Subpoint` children retain their parent's framework-native subtype (`csta:Standard`, `cyberorg:Standard`), so a plain type query catches parsed sub-points alongside genuine parents. CSTA carries 20 Subpoints this way. Cyber.org carries none, so its pool was clean by coincidence rather than by construction.
