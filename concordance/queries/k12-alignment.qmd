...
description: "Pairwise Jaccard similarity between the Cyber.org K-12 parent standards and the CSTA K-12 CS parent standards, with and without each side's clarification-derived examples. The contrast between parent-only and full-document similarity is the methodological finding."
date: 2026-05-09
...
for (p in c("k12_alignment.rds", "k12_alignment_parents.rds")) {
  if (!file.exists(here("_data", p))) {
    stop("Missing _data/", p, ". Run concordance/_data-prep-k12-alignment.R ",
...
knitr::read_chunk(here("_data-prep-k12-alignment.R"))

n_cyberorg <- nrow(alignment)
n_csta <- sum(parents$framework_slug == "csta-2017")
n_parent_hit <- sum(alignment$parent_strength != "none")
n_full_hit <- sum(alignment$full_strength != "none")
n_gained <- n_full_hit - n_parent_hit
n_nomatch <- n_cyberorg - n_full_hit
pct_nomatch <- round(n_nomatch / n_cyberorg * 100)
n_moderate <- sum(alignment$full_strength %in% c("strong", "moderate"))
n_csta_subpoints <- cybedtools::framework_summary$subpoint_count[
  cybedtools::framework_summary$framework_slug == "csta-2017"
]
```
...
The full-document pass roughly doubles the count of detectable matches (`r n_parent_hit` to `r n_full_hit`) over the parent-only pass. None reach the moderate tier (Jaccard ≥ 0.20) under either pass.
...
Compare Cyber.org and CSTA at the parent-statement level only and you find `r n_parent_hit` of `r n_cyberorg` Cyber.org standards with a CSTA match. Compare full documents (parent plus Subpoints plus Examples) and you find `r n_full_hit`. `r n_gained` Cyber.org standards have a plausible CSTA counterpart that is invisible to the parent-only comparison.

### The frameworks are still largely complementary

Even on the full-document pass, `r n_nomatch` of `r n_cyberorg` Cyber.org standards (`r pct_nomatch`%) have no detectable CSTA match. Cyber.org covers cybersecurity-specific territory (threat actors, social engineering, physical security, risk-management foundations) that the CSTA K-12 CS Standards do not address.
...
The `anti_join` is required, not optional. `cybed:Subpoint` children retain their parent's framework-native subtype (`csta:Standard`, `cyberorg:Standard`), so a plain type query catches parsed sub-points alongside genuine parents. CSTA carries `r n_csta_subpoints` Subpoints this way. Cyber.org carries none, so its pool was clean by coincidence rather than by construction.
