...
n_hit <- sum(alignment$full_pct >= 10)
n_nomatch <- n_cyberorg - n_hit
```
...
Of `r n_cyberorg` Cyber.org standards, `r n_hit` share enough vocabulary with a CSTA standard to be worth a closer look (at least 10% overlap). None overlap heavily. That tracks. Cyber.org covers cybersecurity-specific territory that CSTA's general CS scope was never designed to address.
